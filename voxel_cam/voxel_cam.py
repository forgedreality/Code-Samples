...
def beziersetup(firstkeyframe, lastkeyframe, data, totalframes):
    #Setup + Error check
    params = []
    isdupicate = False
    ...
    for frame in range(totalframes):
        ...
        for i in range(len(params)):
            ...
            if key.find('ry') != -1:
                lerparray = beznormalize(lerparray,firstkeyframe,lastkeyframe,data)

            commandstring = commandstring + key + ' ' + str(round(bezier(lerparray, frame/totalframes,key),4)) + ' | '
        ...
        print('Frame: ' + str(frame+1) + ' of ' + str(totalframes))
        ...
        print('Estimated time left: ' + str(round((endtime - starttime) * (totalframes - frame),2)) + ' seconds')
        print('Estimated time left: ' + str(round((endtime - starttime) * (totalframes - frame)/60,2)) + ' minutes')
        print('Estimated time left: ' + str(round((endtime - starttime) * (totalframes - frame) / 3600,2)) + ' hours')

def beznormalize(lerparray,firstkeyframe,lastkeyframe,data):
    newlerparray = []
    newlerparray.append(lerparray[0])
    for i in range(0,lastkeyframe):
...
def readconfig():
    try:
        ...
        print('ERROR: Invalid or no config.json in this folder.')
        exitprog()
    ...
    for i in range(len(data['keyframe'])-1):
        if interpolation[i] == 'linear':
            ...
            totalframes = int(data['keyframe'][i]['option']['frames'])
            for j in range(i+1,len(data['keyframe'])-1):
                if interpolation[j] == 'sequence':
                    totalframes += int(data['keyframe'][j]['option']['frames'])
                else:
                    break
            beziersetup(i, j+1, data, totalframes)
...
def normalizeneg(val):
    return (val % -360 + -360) % -360

def linear(currentkeyframe, data):
    try:
            ...
            for key, _ in data['keyframe'][currentkeyframe]['param'].items():
                ...
                if key.find('ry') != -1: #There should be a function that does normalisation for all parameters that require it not just cam ry / yaw
                    ...
                    if currentDircetion == 'clockwise':
                        ...
                        goalPos = normalize(goalPos)
                    elif currentDircetion == 'counterclockwise':
                        startPos = normalize(startPos)
                        goalPos = normalizeneg(goalPos)
