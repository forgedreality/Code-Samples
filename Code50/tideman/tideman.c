...
bool vote(int rank, string name, int ranks[])
{
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        {
            // set the voter's rank preference
            ranks[rank] = i;
            return true;
        }
    }
...
void record_preferences(int ranks[])
{
    // ranks[i] where i is the index of the candidates
    for (int i = 0; i < candidate_count; i++)
    {
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        for (int j = i + 1; j < candidate_count; j++)
        {
            if (preferences[i][j] != preferences[j][i])
            {
                ...
                // add the pair to pairs array based on who has more votes
...
// check for cycles
bool is_cyclic(int endCase, int start)
{
    if (endCase == start)
    {
        return true;
    ...
    for (int j = 0; j < candidate_count; j++)
    {
        if (locked[endCase][j])
        {
            ...
            // return is_cyclic(j, start);

            // but this one does? D:
            if (is_cyclic(j, start))
            {
                return true;
            }
        }
...
void print_winner(void)
{
    for (int i = 0; i < candidate_count; i++)
    {
        bool edges_to_candidate = false;
        for (int j = 0; j < candidate_count; j++)
        {
            if (locked[j][i])
            {
                edges_to_candidate = true;
                break;
            }
        }
        if (!edges_to_candidate)
        {
            printf("%s\n", candidates[i]);
            return;
        }
